...
var pos = this._currentOffset
  ...
  // small and compact, possible shortcut
  shared      = this._block.buffer[this._currentOffset]
  nonShared   = this._block.buffer[this._currentOffset + 1]
  valueLength = this._block.buffer[this._currentOffset + 2]

  if ((shared | nonShared | valueLength) < 128) {
    this._currentOffset += 3
  } else {
    // alas, they are too big, have to do a full decode
